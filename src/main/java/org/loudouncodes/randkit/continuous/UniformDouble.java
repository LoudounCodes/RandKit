 ...
 * <p>
 * PDF: {@code f(x) = 1 / (b - a)} for {@code x ∈ [a, b)}, else 0.<br>
 * CDF: {@code F(x) = 0} for {@code x ≤ a}; {@code (x - a)/(b - a)} for {@code a < x < b};
 * and {@code 1} for {@code x ≥ b}.
 */
public final class UniformDouble implements ContinuousDistribution {
    ...
    /** {@inheritDoc} */
    @Override
    public double mean() { return 0.5 * (a + b); }

    /** {@inheritDoc} */
    @Override
    public double variance() { return (width * width) / 12.0; }
