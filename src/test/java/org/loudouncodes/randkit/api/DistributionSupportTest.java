...
class DistributionSupportTest {
    ...
    void continuousFactory() {
        DistributionSupport s =
                DistributionSupport.continuous(-2.0, true, 3.5, false);
    ...
    void discreteFactory() {
        DistributionSupport s =
                DistributionSupport.discrete(0.0, true, 10.0, true);
    ...
    void invalidBounds() {
        assertThrows(IllegalArgumentException.class,
                () -> DistributionSupport.continuous(5.0, true, 5.0, true));
        assertThrows(IllegalArgumentException.class,
                () -> DistributionSupport.continuous(6.0, false, 5.0, true));
        assertThrows(IllegalArgumentException.class,
                () -> DistributionSupport.discrete(3.0, true, 2.0, true));
    }

    @Test
    @DisplayName("equals and hashCode reflect all fields")
    void equalsHashCode() {
        DistributionSupport a =
                DistributionSupport.continuous(0.0, true, 1.0, false);
        DistributionSupport b =
                DistributionSupport.continuous(0.0, true, 1.0, false);
        DistributionSupport c =
                DistributionSupport.discrete(0.0, true, 1.0, false);
