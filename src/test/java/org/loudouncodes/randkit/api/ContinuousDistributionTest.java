...
/**
 * Contract checks for the ContinuousDistribution interface using a simple
 * Uniform(a,b) implementation defined locally for tests.
 */
class ContinuousDistributionTest {
    ...
    private static final class TestUniform implements ContinuousDistribution {
        ...
        @Override public double sample() { return a + rng.nextDouble() * width; }
        @Override public double pdf(double x) { return (x >= a && x < b) ? (1.0 / width) : 0.0; }
        @Override public double cdf(double x) {
            if (x <= a) return 0.0;
            if (x >= b) return 1.0;
            return (x - a) / width;
        }
        @Override public double quantile(double p) {
        ...
        @Override public double mean() { return 0.5 * (a + b); }
        @Override public double variance() { return (width * width) / 12.0; }
    }
    ...
    void constructorValidation() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new TestUniform(Double.NEGATIVE_INFINITY, 0.0));
        assertThrows(IllegalArgumentException.class, () -> new TestUniform(0.0, Double.POSITIVE_INFINITY));
    }
